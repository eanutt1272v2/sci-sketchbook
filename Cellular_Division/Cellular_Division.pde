...
void draw() {
   try {
      ...
      text("FPS: " + nf(frameRate, 1, 1), 30, 30);
      int elapsed = (millis() - startMillis) / 1000;
      int seconds = (int) (elapsed % 60);
      int minutes = (int) (elapsed / 60) % 60;
      int hours = (int) (elapsed / 3600);
      text("Time: " + hours + "h " + minutes + "m " + seconds + "s",
      30, 60);
      text("Estimated Cell Count: " + lastCellCount, 30, 90);
      
      textAlign(RIGHT, BASELINE);
      text("GREEN particles are nutrients", width - 30, 30);
      text("BROWN particles are premature spores", width - 30, 60);
