class StatisticsGraph {
  ...
  _teardownStatisticsGraph() {
    ...
    if (
      ...
      this._statisticsGraphWrapper.parentNode
    ) {
      this._statisticsGraphWrapper.parentNode.removeChild(
        this._statisticsGraphWrapper,
      );
    }
...
window.StatisticsGraph = window.StatisticsGraph || {};
window.StatisticsGraph.install = function installStatisticsGraph(targetClass) {
  if (!targetClass || !targetClass.prototype) return;
  for (const name of Object.getOwnPropertyNames(StatisticsGraph.prototype)) {
    if (name === "constructor") continue;
    targetClass.prototype[name] = StatisticsGraph.prototype[name];
  }
};
