...
try:
    reikna_fft = importlib.import_module("reikna.fft")
    reikna_cluda = importlib.import_module(
        "reikna.cluda"
    )  # pip3 install pyopencl/pycuda, reikna
    HAS_REIKNA = True
except ImportError:
    reikna_fft = None
    reikna_cluda = None
...
try:
    ...
    raise RuntimeError("tkinter is required to run LeniaNDKC.py") from exc
from fractions import Fraction
import copy, re, itertools, json, csv
import io, os, sys, argparse, datetime, time, string, subprocess, multiprocessing, shutil
import warnings

warnings.filterwarnings(
    "ignore", ".*output shape of zoom.*"
)  # suppress warning from scipy.ndimage.zoom()
warnings.filterwarnings(
    "ignore", ".*divide by zero encountered.*"
)  # suppress warning from divide by zero in kernel_core()
warnings.filterwarnings(
    "ignore", ".*invalid value encountered.*"
)  # suppress warning from divide by zero in normalize(), get_stat_row()
warnings.filterwarnings(
    "ignore", ".*nperseg.*"
)  # suppress warning from scipy.signal.periodogram/welch
...
parser.add_argument(
    "-g", "--gpu", dest="G", action="store_true", help="interactive choose GPU"
)
...
group.add_argument(
    ...
    type=int,
    nargs="+",
    help="array size = 2^S (apply to all sides if only one value, default 2^(W-D))",
)
group.add_argument(
    "-w",
    "--win",
...
parser.add_argument(
    ...
    action="store",
    type=int,
    help="number of self-connecting kernels (default 1)",
)
parser.add_argument(
    "-x",
    "--cross",
    ...
    action="store",
    type=int,
    help="number of cross-connecting kernels (default 1)",
)
parser.add_argument(
    "-f",
    "--found",
    ...
    action="store",
    type=str,
    help="found animals filename (default <DCK>.json)",
)
...
SKETCHBOOK_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "../../"))
LEGACY_RESOURCE_DIR = os.path.join(SCRIPT_DIR, "resource")
SHARED_DATA_DIR = os.path.join(SKETCHBOOK_DIR, "_shared", "data")
SHARED_FONTS_DIR = os.path.join(SKETCHBOOK_DIR, "_shared", "fonts")
...
class Board:
    ...
    def _recur_join_st(dim, lists, row_func):
        if dim < DIM - 1:
            return DIM_DELIM[DIM - 1 - dim].join(
                Board._recur_join_st(dim + 1, e, row_func) for e in lists
            )
        else:
    ...
    def cells2rle(A, is_shorten=True):
        ...
        if is_shorten:
            rle_groups = Board._recur_drill_list(
                0,
                values,
                lambda row: [
                    (len(list(g)), Board.val2ch(v).strip())
                    for v, g in itertools.groupby(row)
                ],
            )
            st = Board._recur_join_st(
                0,
                rle_groups,
                lambda row: [(str(n) if n > 1 else "") + c for n, c in row],
            )  # "2 yO $ 1 yO"
        else:
            st = Board._recur_join_st(
                0, values, lambda row: [Board.val2ch(v) for v in row]
            )
        return st + "!"
        ...
        for ch in st:
            if ch.isdigit():
                ...
                if last + ch not in delims:
                    Board._append_stack(
                        stacks[0], Board.ch2val(last + ch) / 255, count, is_repeat=True
                    )
                else:
                    dim = delims.index(last + ch)
                    for d in range(dim):
                        Board._append_stack(
                            stacks[d + 1], stacks[d], count, is_repeat=False
                        )
                        stacks[d] = []
    ...
    def _recur_add(self, dim, cells1, cells2, shift, is_centreed):
        size1, size2 = cells1.shape[0], cells2.shape[0]
        size0 = min(size1, size2)
        start1 = (size1 - size0) // 2 + shift[dim] if is_centreed else shift[dim]
        start2 = (size2 - size0) // 2 if is_centreed else 0
        if dim < DIM - 1:
            for x in range(size0):
                self._recur_add(
                    dim + 1,
                    cells1[(start1 + x) % size1],
                    cells2[start2 + x],
                    shift,
                    is_centreed,
                )
    ...
    def add(self, part, shift=[0] * DIM, is_centreed=True):
        # shift: s, z, y, x
        # assert self.params['R'] == part.params['R']
        if type(shift[0]) not in [list]:
            shift = [shift for c in CHANNEL]
        for c in CHANNEL:
            self._recur_add(0, self.cells[c], part.cells[c], shift[c], is_centreed)
        return self
    ...
    def transform(self, tx, mode="RZSF", z_axis=Z_AXIS, is_world=False):
        if "R" in mode and tx["rotate"] != [0] * 3:
            for c in CHANNEL:
                if DIM == 2:
                    self.cells[c] = scipy.ndimage.rotate(
                        self.cells[c],
                        -tx["rotate"][1],
                        reshape=not is_world,
                        order=0,
                        mode="wrap" if is_world else "constant",
                    )
                elif DIM >= 3:
                    self.cells[c] = scipy.ndimage.rotate(
                        self.cells[c],
                    ...
                    self.cells[c] = scipy.ndimage.rotate(
                        ...
                        order=0,
                        mode="wrap" if is_world else "constant",
                    )  # rotate by z axis = y-x plane
        if "Z" in mode and tx["R"] != self.params[0]["R"]:
            # print("* {} / {}".format(tx['R'], self.params['R']))
            shape_orig = self.cells[0].shape
            for c in CHANNEL:
                self.cells[c] = scipy.ndimage.zoom(
                    self.cells[c], tx["R"] / self.params[0]["R"], order=0
                )
            if is_world:
        ...
        if "F" in mode and tx["flip"] != -1:
            ...
            for c in CHANNEL:
                if tx["flip"] in [0, 1]:
                    ...
                    self.cells[c][tuple(extra_slice + slice1)] = self.cells[c][
                        tuple(extra_slice + slice2)
                    ]
                elif tx["flip"] == 3:
                    slice1 = [slice(None), slice(None, -MIDX - 1, -1)]
                ...
                # elif tx['flip'] == 4: i_upper = np.triu_indices(SIZEX, -1); self.cells[i_upper] = self.cells.T[i_upper]
                    ...
                    self.cells[c][
                        tuple(extra_slice + [slice(None), slice(MIDX, None)])
                    ] = 0
                elif tx["flip"] == 5:
                    self.cells[c][
                        tuple(extra_slice + [slice(MIDY, None), slice(None)])
                    ] = 0
        if "S" in mode and tx["shift"] != [0] * DIM:
            for c in CHANNEL:
                self.cells[c] = scipy.ndimage.shift(
                    self.cells[c], tx["shift"], order=0, mode="wrap"
                )
...
class Automaton:
    kernel_core = {
        ...
        3: lambda r, q=1 / 4: (r >= q) * (r <= 1 - q)
        + (r < q) * 0.5,  # staircase (life)
    }
    growth_func = {
        0: lambda n, m, s: np.maximum(0, 1 - (n - m) ** 2 / (9 * s**2)) ** 4 * 2
        - 1,  # polynomial (quad4)
        1: lambda n, m, s: np.exp(-((n - m) ** 2) / (2 * s**2)) * 2
    ...
    def compile_gpu(self, A):
        ...
        self.gpu_api = self.gpu_thr = self.gpu_fft1 = self.gpu_fftn = (
            self.gpu_fftshift
        ) = None
        if not HAS_REIKNA:
            self.has_gpu = False
            ...
            return
        try:
            self.gpu_api = reikna_cluda.any_api()
            self.gpu_thr = self.gpu_api.Thread.create(interactive=args.G)
            self.gpu_fft1 = reikna_fft.FFT(A.astype(np.complex64), axes=[0]).compile(
                self.gpu_thr
            )
            self.gpu_fftn = reikna_fft.FFT(A.astype(np.complex64)).compile(self.gpu_thr)
            self.gpu_fftshift = reikna_fft.FFTShift(A.astype(np.float32)).compile(
                self.gpu_thr
            )
    ...
    def calc_once(self, is_update=True):
        ...
        for k in KERNEL:
            ...
            self.potential[k] = self.fftshift(
                np.real(self.ifftn(self.potential_FFT[k]))
            )
        ...
        for c in CHANNEL:
            if self.add_noise > 0:
                rand = (np.random.random_sample(A_new[c].shape) - 0.5) * (
                    self.add_noise / 10
                ) + 1
                A_new[c] *= rand
            if self.is_soft_clip:
                A_new[c] = Automaton.soft_clip(
                    A_new[c], 0, 1, 1 / dt
            ...
            if is_update:
                if self.mask_rate > 0:
                    mask = np.random.random_sample(A_new[c].shape) > (
                        self.mask_rate / 10
                    )
...
class Analyzer:
    STAT_NAMES = {
        ...
        "m": "Mass (mg)",
        "g": "Growth (mg/s)",
        "r": "Gyradius (mm)",  # 'I':'Moment of inertia'
    ...
    def get_stat_row(self):
        ...
        if self.m_centre is not None:
            pos = self.m_centre * R + self.total_shift_idx
        else:
            pos = [0] * DIM
        RN = np.power(R, DIM)
        return [
            ...
            self.growth / RN,
            (
                np.sqrt(self.inertia / self.mass) if self.mass != 0 else 0
            ),  # self.inertia/RN  # self.inertia*RN,
            self.mg_dist,
            self.m_shift * T,
            self.m_rotate * T,
            self.mass_asym / RN,
    ...
    def calc_polar_FFT(self, polar_array, is_gaussian_blur=True):
        if is_gaussian_blur and PIXEL > 1:
            polar_array[:SIZER, :] = scipy.ndimage.filters.gaussian_filter(
                polar_array[:SIZER, :], sigma=(2, 1)
            )
    ...
    def calc_statistics(self, polar_what=0, psd_x="m", psd_y="g", is_welch=True):
        ...
        if m0 > EPSILON:
            ...
            MX = self.m_centre = np.asarray(MX1) / m0
            ...
            if g0 > EPSILON:
                ...
                GX = self.g_centre = np.asarray(GX1) / g0
                self.mg_dist = np.linalg.norm(self.m_centre - self.g_centre)

            if self.m_last_centre is not None and self.m_last_angle is not None:
                u = self.m_centre
                v = self.m_last_centre - self.last_shift_idx / R
                dm = u - v
                self.m_shift = np.linalg.norm(dm)
                self.m_angle = (
                    np.degrees(np.arctan2(dm[1], dm[0]))
                    if self.m_shift >= EPSILON
                    else 0
                )
                ...
                if DIM == 2:
                    ...
                    x0, y0 = self.m_last_centre * R + midpoint - self.last_shift_idx
                    x1, y1 = self.m_centre * R + midpoint
            ...
            if self.is_calc_symmetry:
                ...
                sides_row[0] = 1
                self.polar_FFT = self.calc_polar_FFT(
                    self.polar_array, is_gaussian_blur=True
                )
                ...
                if self.density_ema is not None:
                    self.density_ema = self.density_ema + self.ema_alpha * (
                        self.density_sum - self.density_ema
                    )
                else:
                ...
                if self.last_polar_angle is not None:
                    if self.last_shift_idx[0] == self.last_shift_idx[1] == 0:
                        ...
                        polar_array_unshift = A2[
                            (self.automaton.polar_Y - self.last_shift_idx[1]) % SIZEY,
                            (self.automaton.polar_X - self.last_shift_idx[0]) % SIZEX,
                        ]
                        polar_FFT_unshift = self.calc_polar_FFT(
                            polar_array_unshift, is_gaussian_blur=True
                        )
                    ...
                    # max_angle = np.pi
                    self.polar_rotate = (self.polar_rotate + 3 * max_angle) % (
                        2 * max_angle
                    ) - max_angle

                    self.polar_rotate2 = self.polar_angle - self.last_polar_angle
                    self.polar_rotate2 = (self.polar_rotate2 + 3 * max_angle) % (
                        2 * max_angle
                    ) - max_angle
            ...
            if self.is_calc_psd:
                ...
                if self.series != [] and segment != []:
                    if self.automaton.gen % self.PSD_INTERVAL == 0:
                        ...
                        self.psd_freq, self.psd1 = self.calc_psd(
                            X, fs=T, nfft=512, is_welch=is_welch
                        )
                        _, self.psd2 = self.calc_psd(
                            Y, fs=T, nfft=512, is_welch=is_welch
                        )
    ...
    def invalidate_segment(self):
        if self.series != []:
            self.series[-1] = [
                [self.world.params[0]["m"], self.world.params[0]["s"]]
                + [np.nan] * (len(self.STAT_HEADERS) - 2)
            ]
            self.new_segment()
    ...
    def centre_world(self):
        if self.mass < EPSILON or self.m_centre is None:
            return
        axes = tuple(reversed(range(DIM)))
        self.last_shift_idx = (self.m_centre * self.world.params[0]["R"]).astype(int)
        self.total_shift_idx += self.last_shift_idx
        self.world.cells = [
            np.roll(self.world.cells[c], -self.last_shift_idx, axes) for c in CHANNEL
        ]
        self.automaton.potential = [
            np.roll(self.automaton.potential[k], -self.last_shift_idx, axes)
            for k in KERNEL
        ]
        self.automaton.field = [
            np.roll(self.automaton.field[k], -self.last_shift_idx, axes) for k in KERNEL
        ]
        self.automaton.change = [
            np.roll(self.automaton.change[c], -self.last_shift_idx, axes)
            for c in CHANNEL
        ]
...
class Recorder:
    RECORD_ROOT = os.path.join(SCRIPT_DIR, "record")
    FRAME_EXT = ".png"
    VIDEO_EXT = ".mp4"
    GIF_EXT = ".gif"
    TIF_EXT = ".tif"
    ...
    ffmpeg_cmd = [
        ...
        "rgb24",  # input options
        "-s",
        "{x}x{y}".format(x=SIZEX * PIXEL, y=SIZEY * PIXEL),
        "-r",
        str(VIDEO_FPS),
        "-i",
        "{input}",  # input pipe
        ...
        "yuv420p",
        "-crf",
        "1",  # output options (high quality H.264 encoding)
        "{output}",
    ]  # output file
    ...
    def start_record(self):
        ...
        STATUS.append(
            "> start "
            + ("saving frames" if self.is_save_frames else "recording video")
            + " and GIF..."
        )
        os.makedirs(self.RECORD_ROOT, exist_ok=True)
        self.record_id = "{name}-{time}".format(
            name=self.world.names[0].split("(")[0],
            time=datetime.datetime.now().strftime("%Y%m%d-%H%M%S"),
        )
        self.record_seq = 1
        self.video_path = os.path.join(
            self.RECORD_ROOT, self.record_id + self.VIDEO_EXT
        )
        ...
        if self.is_save_frames:
            ...
            ffmpeg_bin = shutil.which("ffmpeg") or self.ffmpeg_cmd[0]
            cmd = [
                s.replace("{input}", "-").replace("{output}", self.video_path)
    ...
    def save_image(self, img, filename=None):
        ...
        os.makedirs(self.RECORD_ROOT, exist_ok=True)
        img_path = (
            filename + self.FRAME_EXT
            if filename
            else os.path.join(self.RECORD_ROOT, self.record_id + self.FRAME_EXT)
        )
        img.save(img_path)
    ...
    def record_frame(self, img):
        if self.is_save_frames:
            img_path = os.path.join(
                self.RECORD_ROOT,
                self.record_id,
                "{:03d}".format(self.record_seq) + self.FRAME_EXT,
            )
            img.save(img_path)
        else:
            if self.video:
    ...
    def finish_record(self):
        ...
        if self.is_save_frames:
            STATUS.append(
                "> frames saved to '" + self.img_dir + "/*" + self.FRAME_EXT + "'"
            )
            cmd = [
                s.replace(
                    "{input}", os.path.join(self.img_dir, "%03d" + self.FRAME_EXT)
...
class Lenia:
    ...
    ANIMALS_FILENAME = "animals.json" if DIM == 2 else "animals" + str(DIM) + "D.json"
    ANIMALS_PATH = first_existing_path(
        os.path.join(SCRIPT_DIR, ANIMALS_FILENAME),
        os.path.join(SHARED_DATA_DIR, ANIMALS_FILENAME),
    )
    found_path = args.F
    if found_path is None:
        found_path = "{D}{C}{K}{X}.json".format(
            D=DIM, C=CN, K=KN, X="" if XN == 1 else str(XN)
        )
    elif not found_path.endswith(".json"):
        found_path += ".json"
    FOUND_ANIMALS_PATH = os.path.join(SCRIPT_DIR, "found", found_path)
    ...
    def __init__(self):
        ...
        self.colormaps = [
            ...
            self.create_colormap(
                np.asarray([[8, 8, 8], [7, 7, 7], [5, 5, 5], [3, 3, 3], [0, 0, 0]]),
                is_marker_w=False,
            ),  # W/B
            self.create_colormap(
                np.asarray([[0, 0, 0], [3, 3, 3], [5, 5, 5], [7, 7, 7], [8, 8, 8]])
            ),
        ]  # B/W
        ...
        self.channelmaps = (
            np.asarray(
                [
                    [8, 0, 0],
                    [0, 8, 0],
                    [0, 0, 8],  # RGB = Red/Green/Blue
                    [6, 3, 0],
                    [0, 6, 3],
                    [3, 0, 6],  # OTV = Orange/Turquoise/Violet
                    [4, 4, 4],
                    [3, 3, 3],
                    [2, 2, 2],  # Greyscale
                    [8, 0, 0],
        ...
        self.channelbg = (
            np.asarray(
                [[0, 0, 2], [0, 1, 0], [0, 0, 0], [0, 0, 2], [0, 0, 2], [0, 0, 2]]
            )
            / 8
        )  # BG [0,0,2]
    ...
    def _load_ui_font(self):
        ...
        if os.path.exists(legacy_bitmap):
            try:
                ...
                pass
        if os.path.exists(modern_ttf):
            try:
                return PIL.ImageFont.truetype(modern_ttf, 12)
            except Exception:
                pass
        return PIL.ImageFont.load_default()

    @property
    def statistics_x_name(self):
        return self.analyzer.STAT_HEADERS[self.statistics_x]

    @property
    def statistics_y_name(self):
        return self.analyzer.STAT_HEADERS[self.statistics_y]

    @statistics_x_name.setter
    def statistics_x_name(self, val):
        self.statistics_x = self.analyzer.STAT_HEADERS.index(val)

    @statistics_y_name.setter
    def statistics_y_name(self, val):
        self.statistics_y = self.analyzer.STAT_HEADERS.index(val)
    ...
    def convert_font_run_once(self, font_file_path):
        ...
        with open(font_file_path, "rb") as fp:
            p = PIL.BdfFontFile.BdfFontFile(
                fp
            )  # PcfFontFile if you're reading PCF files
    ...
    def read_animals(self):
        try:
            with open(self.ANIMALS_PATH, "r", encoding="utf-8") as file:
                ...
                new_animal_data = [
                    line for line in new_animal_data if type(line) in [dict]
                ]
    ...
    def read_found_animals(self):
        try:
            with open(self.FOUND_ANIMALS_PATH, "r", encoding="utf-8") as file:
                ...
                new_found_animal_data = [
                    line for line in new_found_animal_data if type(line) in [dict]
                ]
                self.found_animal_data = new_found_animal_data
                self.found_animal_id -= self.deleted_found_animal_count
                self.deleted_found_animal_count = 0
                STATUS.append(
                    "> found lifeforms loaded from " + self.FOUND_ANIMALS_PATH
                )
        except IOError:
            pass
        except json.JSONDecodeError as e:
    ...
    def load_animal_id(self, world, id, **kwargs):
        ...
        self.load_part(
            world, Board.from_data(self.animal_data[self.animal_id]), **kwargs
        )
        self.last_load_animal = True
    ...
    def load_found_animal_id(self, world, id, **kwargs):
        ...
        self.load_part(
            world,
            Board.from_data(self.found_animal_data[self.found_animal_id]),
            is_use_part_R=True,
            **kwargs
        )
    ...
    def get_animal_id(self, code):
        ...
        itr = (
            id
            for (id, data) in enumerate(self.animal_data)
            if data["code"] == code_sp[0]
        )
        for i in range(n):
            id = next(itr, None)
        return id
    ...
    def get_found_animal_id(self, code):
        ...
        itr = (
            id
            for (id, data) in enumerate(self.found_animal_data)
            if data["code"] == code_sp[0] or data["code"].startswith(code_sp[0] + "<")
        )
        id = next(itr, None)
        return id
        ...
        if part.cells is not None:
            ...
            for i in range(repeat):
                if is_random:
                    ...
                    self.tx["shift"] = [
                        np.random.randint(d1 + d0) - d1 // 2
                        for d0, d1 in zip(shape0, shape1)
                    ]
        ...
        if self.is_layer_mode:
            if self.back is not None:
                ...
                self.world.transform(
                    self.tx, mode="Z", z_axis=self.z_axis, is_world=True
                )
            ...
            if self.tx["flip"] < 2:
                ...
                self.world.transform(
                    self.tx, mode="RS", z_axis=self.z_axis, is_world=True
                )
                self.world.transform(
                    self.tx, mode="ZF", z_axis=self.z_axis, is_world=True
                )
            self.automaton.calc_kernel()
    ...
    def random_world(self, is_reseed=False, is_fill=False):
        ...
        if is_fill:
            ...
            rand = [
                np.random.random_sample(tuple(reversed(dims))) * 0.9 for c in CHANNEL
            ]
            self.world.clear()
            ...
            for i in range(np.random.randint(15, 40)):
                rand = [
                    np.random.random_sample(tuple(reversed(dims))) * 0.9
                    for c in CHANNEL
                ]
                border = int(R * 1.5)
                shift = [
                    [
                        (
                            np.random.randint(border, size - border) - size // 2
                            if border < size - border
                            else 0
                        )
                        for size in reversed(SIZE)
                    ]
                    for c in CHANNEL
                ]
        ...
        if is_incremental:  # local
            ...
            for k in KERNEL:
                    ...
                    for bi in range(len(p["b"])):
                        ...
                        if rand == 0:
                            p["b"][bi] = max(
                                ...
                                min(
                                    1,
                                    p["b"][bi] + Fraction(np.random.randint(3) - 1, 12),
                                ),
                            )
                    if max(p["b"]) != 1:
                ...
                self.random_local(
                    p,
                    "m",
                    delta=0.02 if is_small else 0.05,
                    vmin=0.1,
                    vmax=1.0,
                    digit=3,
                )
                self.random_local(
                    p,
                    "s",
                    delta=0.002 if is_small else 0.005,
                    vmin=0.01,
                    vmax=1.00,
                    digit=4,
                )
            ...
            for k in KERNEL:
                ...
                self.random_global_mul(
                    p, "s", "m", dmin=1 / 2, dmax=3, digit=4
                )  # 1/1.5, 2.5 | 1, 5
    ...
    def put_world_in_leaderboard(self):
        ...
        cname = "{func}({stat})={fitness:.3f}".format(
            func=func, stat=self.statistics_x_name, fitness=fitness
        )
        self.world.names[2] = cname
        # self.update_lineage(cname=cname)
        self.leaderboard.append(
            {"fitness": fitness, "world": copy.deepcopy(self.world)}
        )
            ...
            for i in order:
                ...
                if world is not None:
                    if w1 is None:
                        ...
                        self.world.params = []
                        self.world.names = [
                            self.clean_code(w1.names[0])
                            + "x"
                            + self.clean_code(w2.names[0]),
                            "",
                            "",
                        ]
                        self.update_lineage()
    ...
    def do_search(self):
        ...
        test_long = self.search_algo in [1] or (
            self.search_algo in [3] and self.breadth_count == 3
        )
        test_short = self.search_algo in [2] or (
            self.search_algo in [3] and self.breadth_count < 3
        )
        ...
        if self.search_mode == +1:
            ...
            if self.analyzer.is_empty or self.analyzer.is_full:
                ...
                (test_vshort and self.automaton.gen >= 250)
                or (test_short and self.automaton.gen >= 500)
                or (test_long and self.automaton.gen >= 750)
            ):
                self.is_show_search = True
                self.search_stage = 1
                self.search_total += 1
                ...
                self.search_algo in [4, 5, 6]
                and self.automaton.gen >= 250
                and self.search_stage == 1
            ):
                self.is_show_search = True
                self.search_stage = 2
                self.automaton.reset()
            ...
            if self.is_show_search:
                if self.search_algo in [0, 1, 2, 3]:
                    STATUS.append(
                        "{success} found in {trial} trials ({algo}), saving to {path}".format(
                            success=self.search_success,
                            trial=self.search_total,
                            algo=self.get_value_text("search_algo"),
                            path=self.FOUND_ANIMALS_PATH,
                        )
                    )
                elif self.search_algo in [4, 5, 6]:
                    STATUS.append(
                        "{leader1:.3f}, {leader2:.3f}, {leader3:.3f} leading in {trial} steps ({algo})".format(
                            stat=self.statistics_x_name,
                            leader1=self.leaderboard[0]["fitness"],
                            leader2=self.leaderboard[1]["fitness"],
                            leader3=self.leaderboard[2]["fitness"],
                            trial=self.search_total,
                            algo=self.get_value_text("search_algo"),
                        )
                    )
    ...
    def append_found_file(self, world=None, newline=",\n"):
        ...
        try:
            os.makedirs(os.path.dirname(self.FOUND_ANIMALS_PATH), exist_ok=True)
    ...
    def append_found_file_leaderboard(self):
        ...
        self.append_found_file_text(
            '"Leaderboard: {algo} where {stat}",\n'.format(
                stat=self.analyzer.statistics_fullname(i=self.statistics_x),
                algo=self.get_value_text("search_algo"),
            )
        )
    ...
    def create_window(self):
        ...
        icon_path = os.path.join(LEGACY_RESOURCE_DIR, "icon" + str(icon_no) + ".png")
    ...
    def create_panel(self, c, r, w, h):
        ...
        img = PIL.Image.frombuffer(
            "P", (w * PIXEL, h * PIXEL), buffer, "raw", "P", 0, 1
        )
    ...
    def create_colormap(self, colors, is_marker_w=True):
        ...
        return np.rint(c / 8 * 255).astype(int).tolist() + (
            self.MARKER_COLORS_W if is_marker_w else self.MARKER_COLORS_B
        )
    ...
    def create_colormap_flat(self, c0, c1, is_marker_w=True):
        ...
        c = np.clip(c, 0, 255)
        return np.rint(c.flatten("F")).astype(int).tolist() + (
            self.MARKER_COLORS_W if is_marker_w else self.MARKER_COLORS_B
        )
    ...
    def create_colormap_turbo(self, seq="rgb", is_marker_w=True):
        ...
        s["b"] = 27.2 + x * (
            3211.1 - x * (15327.97 - x * (27814 - x * (22569.18 - x * 6838.66)))
        )
    ...
    def update_window(self, show_arr=None, is_reimage=True):
        if is_reimage:
            ...
            if show_arr is not None:
                ...
                if self.show_what == 0:
                    self.draw_world(
                        self.show_which_channels(self.world.cells),
                        0,
                        1,
                        is_shift=True,
                        is_higher_zero=True,
                    ...
                    self.draw_world(
                        self.automaton.potential[self.show_kernel],
                        0,
                        2 * self.world.params[self.show_kernel]["m"],
                        is_shift=True,
                        is_higher_zero=True,
                        markers=["arrow", "scale", "grid", "colormap", "params"],
                    )
                elif self.show_what == 2:
                    self.draw_world(
                        self.automaton.field[self.show_kernel],
                        field_lo,
                        1,
                        is_shift=True,
                        markers=["arrow", "scale", "grid", "colormap", "params"],
                    ...
                    self.draw_world(
                        self.automaton.kernel[self.show_kernel],
                        0,
                        1,
                        markers=["scale", "fixgrid", "colormap", "params"],
                    )
                ...
                self.draw_statistics(
                    is_current_series=self.statistics_mode in [1, 2, 3],
                    is_small=self.statistics_mode in [1],
                )
            ...
            if self.is_save_image:
                ...
                self.recorder.save_image(
                    self.img, filename=os.path.join(self.SAVE_ROOT, str(self.file_seq))
                )
                self.is_save_image = False
    ...
    def get_image(self, buffer):
        ...
        if self.is_show_rgb():
            ...
            return PIL.Image.frombuffer(
                "RGB", (x * PIXEL, y * PIXEL), buffer, "raw", "RGB", 0, 1
            )
        else:
            return PIL.Image.frombuffer(
                "P", (x * PIXEL, y * PIXEL), buffer[0], "raw", "P", 0, 1
            )
    ...
    def draw_world(
        self, A, vmin=0, vmax=1, is_shift=False, is_higher_zero=False, markers=[]
    ):
        ...
        R = self.world.params[0]["R"]
        is_xy = (
            self.statistics_x_name in ["x"]
            and self.statistics_y_name in ["y"]
            and self.statistics_mode in [2]
        )
        axes = tuple(reversed(range(DIM)))
        if is_shift and not self.is_auto_centre:
            shift = (
                self.analyzer.total_shift_idx
            )  # if 'world' in markers else self.analyzer.total_shift_idx - self.analyzer.last_shift_idx
        ...
        if self.polar_mode in [0, 1]:
            ...
            if self.is_show_rgb():
                buffer = [
                    np.uint8(np.clip(self.normalize(A0, vmin, vmax), 0, 1) * 252)
                    for A0 in A
                ]
                self.img = self.get_image(buffer)
            else:
                buffer = np.uint8(
                    np.clip(self.normalize(A[0], vmin, vmax), 0, 1) * 252
                )  # .copy(order='C')
            ...
            if (
                PIXEL > 1
                and self.is_auto_centre
                and is_shift
                and self.analyzer.m_centre is not None
            ):
            ...
            if angle_shift != 0:
                self.img = self.img.rotate(
                    -angle_shift * 360, resample=PIL.Image.NEAREST, expand=False
                )
            ...
            if self.polar_mode in [2] and self.analyzer.is_calc_symmetry:
                ...
                if k > 0:
                    ...
                    X_intp = np.interp(
                        np.linspace(0, 1, p * k), np.linspace(0, 1, SIZETH), X
                    )  # np.linspace(0, 1, SIZEX)+a/2/np.pi , +a/2/np.pi-int(a/2/np.pi*SIZEX)/SIZEX
                    X_stack = np.asarray(np.hsplit(X_intp, k))
                    for i in range(k):
                        A2[3 + i, :p] = X_stack[i, :] / X_max
                buffer = np.uint8(np.clip(self.normalize(A2, vmin, vmax), 0, 1) * 252)
            elif (
                self.polar_mode in [3]
                and self.analyzer.polar_array is not None
                and self.analyzer.series_TH is not None
            ):
                ...
                A2 = np.vstack(
                    (
                        self.analyzer.polar_density
                        / np.amax(self.analyzer.polar_density),
                        self.analyzer.rotate_wsum / np.amax(self.analyzer.rotate_wsum),
                    )
                )
                A2[:2, :] = self.analyzer.density_sum[:] / np.amax(
                    self.analyzer.density_sum
                )
                buffer = np.uint8(np.clip(self.normalize(A2, vmin, vmax), 0, 1) * 252)
        ...
        try:
            ...
            try:
                ...
                if hasattr(font, "getbbox"):
                    ...
                    title_w, title_h = len(title) * 6, 10
        title_x, title_y = MIDX * PIXEL - title_w // 2, line * 12 + 7
        draw.text((title_x, title_y), title, fill=self.get_color(color), font=font)
    ...
    def draw_black(self):
        ...
        if self.is_show_rgb():
            self.img = PIL.Image.frombuffer(
                "RGB", isize, np.zeros(asize), "raw", "RGB", 0, 1
            )
        else:
            self.img = PIL.Image.frombuffer(
                "L", isize, np.zeros(asize), "raw", "L", 0, 1
            )
    ...
    def draw_grid(self, buffer, markers=[], is_fixed=False):
        ...
        # n = 2
        if ("grid" in markers or "fixgrid" in markers) and self.markers_mode in [
            0,
            1,
            2,
            3,
            ...
            for i in range(-n, n + 1):
                sx, sy = 0, 0
                if self.is_auto_centre and not is_fixed:
                    sx, sy, *_ = self.analyzer.total_shift_idx.astype(int)
                grid = buffer[
                    (MIDY - sy + i) % R : SIZEY : R, (MIDX - sx) % R : SIZEX : R
                ]
                grid[grid == 0] = 253
                grid = buffer[
                    (MIDY - sy) % R : SIZEY : R, (MIDX - sx + i) % R : SIZEX : R
                ]
                grid[grid == 0] = 253
    ...
    def draw_arrow(self, markers=[]):
        ...
        if (
            "arrow" in markers
            and self.markers_mode in [0, 2, 4, 6]
            and self.polar_mode in [0, 1]
            and R > 2
            and self.analyzer.m_last_centre is not None
            and self.analyzer.m_centre is not None
        ):
            shift = (
                self.analyzer.total_shift_idx
                if not self.is_auto_centre
                else np.zeros(DIM)
            )
            m0 = (
                self.analyzer.m_last_centre * R
                + midpoint
                + shift
                - self.analyzer.last_shift_idx
            )
            m1 = self.analyzer.m_centre * R + midpoint + shift
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    ...
                    p2 = (m3 + adj) * PIXEL
                    draw.line(
                        [p1[0], p1[1], p2[0], p2[1]], fill=self.get_color(254), width=1
                    )
                    for m, c in [(m0, 254), (m1, 255), (m2, 255), (m3, 255)]:
                        p1 = (m + adj) * PIXEL - dd
                        p2 = (m + adj) * PIXEL + dd
                        draw.ellipse(
                            [p1[0], p1[1], p2[0], p2[1]], fill=self.get_color(c)
                        )
        del draw
    ...
    def draw_symmetry_title(self, markers=[]):
        ...
        R, T = [self.world.params[0][k] for k in ("R", "T")]
        if (
            self.analyzer.is_calc_symmetry
            and "scale" in markers
            and self.markers_mode in [0, 2, 4, 6]
            and R > 2
        ):
            k = self.analyzer.symm_sides
            self.draw_title(
                draw,
                0,
                "symmetry: {k} ({name})".format(
                    k=k, name=self.POLYGON_NAME[k] if k <= 10 else self.POLYGON_NAME[0]
                ),
            )
        del draw
    ...
    def draw_symmetry(self, markers=[]):
        ...
        if (
            self.analyzer.is_calc_symmetry
            and "arrow" in markers
            and self.markers_mode in [0, 1, 2, 3]
            and R > 2
            and self.analyzer.m_last_centre is not None
            and self.analyzer.m_centre is not None
        ):
            is_draw_radial = self.polar_mode in [1] or (
                self.polar_mode in [0] and self.auto_rotate_mode in [2]
            )
            shift = (
                self.analyzer.total_shift_idx
                if not self.is_auto_centre
            ...
            if self.analyzer.sides_vec is not None:
                ...
                if is_draw_radial or self.polar_mode in [2]:
                    ...
                    if k > 1:
                        for i in range(k):
                            if is_draw_radial:
                                # radial
                                angle = 2 * np.pi * i / k + a
                                d1 = np.asarray([np.sin(angle), np.cos(angle)]) * max(
                                    SIZEX, SIZEY
                                )
                                draw.line(
                                    tuple(m1 * PIXEL) + tuple((m1 - d1) * PIXEL),
                                    fill=self.get_color(254),
                                    width=1,
                                )
                                ...
                                draw.line(
                                    (x * PIXEL, 0 * PIXEL, x * PIXEL, SIZEY * PIXEL),
                                    fill=self.get_color(254),
                                    width=1,
                                )
                    ...
                    for i in range(1, SIZEF, 5):
                        draw.line(
                            (i * 2 * PIXEL, 0 * PIXEL, i * 2 * PIXEL, SIZEY * PIXEL),
                            fill=self.get_color(254),
                            width=1,
                        )
                        x0, y0 = i * 2 * PIXEL + 2, MIDY * PIXEL
                        draw.text(
                            (x0, y0), str(i), fill=self.get_color(255), font=self.font
                        )
                if self.polar_mode in [2, 3, 4]:
                    # draw horizontal line separate upper and lower panels
                    draw.line(
                        (0 * PIXEL, SIZER * PIXEL, SIZEX * PIXEL, SIZER * PIXEL),
                        fill=self.get_color(254),
                        width=1,
                    )
                    ...
                    if kk[r] > 1:
                        if is_draw_radial:
                            ...
                            for i in range(kk[r]):
                                ...
                                d1 = np.asarray([np.sin(angle), np.cos(angle)]) * (
                                    SIZER - r
                                )
                                ...
                                if th1 > th2:
                                    th1, th2 = th2, th1
                                draw.arc(
                                    tuple((m1 - SIZER + r) * PIXEL)
                                    + tuple((m1 + SIZER - r) * PIXEL),
                                    th1,
                                    th2,
                                    fill=self.get_color(c),
                                    width=1,
                                )
                                draw.ellipse(
                                    tuple((m1 - d1) * PIXEL - dd)
                                    + tuple((m1 - d1) * PIXEL + dd),
                                    fill=self.get_color(c),
                                )
                        elif self.polar_mode in [2]:
    ...
    def draw_legend(self, markers=[], vmin=0, vmax=1):
        ...
        def draw_rect(xa, ya, xb, yb, **kwargs):
            left, right = sorted((int(round(xa)), int(round(xb))))
            top, bottom = sorted((int(round(ya)), int(round(yb))))
        ...
        if (
            ...
            for m, c in [(0, 254), (-10, 255), (-50, 255), (-90, 255)]:
                draw.ellipse(
                    tuple((x0 + m, y0) - dd) + tuple((x0 + m, y0) + dd),
                    fill=self.get_color(c),
                )
            draw.text(
                (x0 - 95, y0 - 20), "2s", fill=self.get_color(255), font=self.font
            )
            draw.text(
                (x0 - 55, y0 - 20), "1s", fill=self.get_color(255), font=self.font
            )
        if (
            "scale" in markers
            and self.markers_mode in [0, 1, 4, 5]
            and self.polar_mode in [0, 1]
        ):
            # draw scale bar
            x0, y0 = SIZEX * PIXEL - 50, SIZEY * PIXEL - 20
            draw.text((x0 + 10, y0), "1mm", fill=self.get_color(255), font=self.font)
            draw_rect(x0 - R * PIXEL, y0 + 3, x0, y0 + 7, fill=self.get_color(255))
        if (
            "colormap" in markers
            and self.markers_mode in [0, 1, 4, 5]
            and self.polar_mode in [0, 1]
            ...
            if self.is_show_rgb():
                ...
                for c in range(ncol):
                    draw_rect(
                        x0, y0 + dy * c, x1, y0 + dy * (c + 1), fill=self.get_color(c)
                    )
            draw_rect(x0 - 1, y0 + 1, x1 + 1, y1 - 1, outline=self.get_color(254))
            draw.text(
                (x0 - 25, y0 - 5),
                "{:.1f}".format(vmin),
                fill=self.get_color(255),
                font=self.font,
            )
            draw.text(
                (x0 - 25, (y1 + y0) // 2 - 5),
                "{:.1f}".format((vmin + vmax) / 2),
                fill=self.get_color(255),
                font=self.font,
            )
            draw.text(
    ...
    def draw_statistics(
        ...
        is_square = self.statistics_x_name in ["x", "y"] and self.statistics_y_name in [
            "x",
            "y",
        ]
        is_xy = (
        ...
        if series != [] and series != [[]]:
            X = [
                np.asarray([val[self.statistics_x] for val in seg])
                for seg in series
                if len(seg) > 0
            ]
            Y = [
                np.asarray([val[self.statistics_y] for val in seg])
                for seg in series
                if len(seg) > 0
            ]
            ...
            xmin, xmax = min(seg.min() for seg in X if seg.size > 0), max(
                seg.max() for seg in X if seg.size > 0
            )
            ymin, ymax = min(seg.min() for seg in Y if seg.size > 0), max(
                seg.max() for seg in Y if seg.size > 0
            )
            ...
            title_st_x = "X: {name} ({min:.3f}-{max:.3f}) {val:.3f}".format(
                name=self.statistics_x_name,
                min=xmin,
                max=xmax,
                val=current[self.statistics_x],
            )
            title_st_y = "Y: {name} ({min:.3f}-{max:.3f}) {val:.3f}".format(
                name=self.statistics_y_name,
                min=ymin,
                max=ymax,
                val=current[self.statistics_y],
            )
            if is_small:
            ...
            if is_draw_line:
                ...
                for x, y, m, s, c in zip(X, Y, M, S, C):
                    ...
                    if self.is_group_params:
                        ...
                        x, y = self.normalize(x, xmin, xmax), self.normalize(
                            y, ymin, ymax
                        )
                        s, m = self.normalize(s, smin, smax + ds), self.normalize(
                            m, mmin, mmax + dm
                        )
                        x, x0, x1 = [
                            (a * ds / (smax - smin + ds) + s) * (SIZEX * PIXEL - 10) + 5
                            for a in [x, 0, 1]
                        ]
                        y, y0, y1 = [
                            (1 - a * dm / (mmax - mmin + dm) - m) * (SIZEY * PIXEL - 10)
                            + 5
                            for a in [y, 0, 1]
                        ]
                        draw.rectangle(
                            [(x0, y0), (x1, y1)],
                            outline=self.get_color(c),
                            fill=None if is_valid else c,
                        )
                    else:
                        if is_xy and self.analyzer.m_centre is not None:
                            midpoint = np.asarray(MID)
                            shift = (
                                self.analyzer.total_shift_idx
                                + self.analyzer.m_centre * R
                                if self.is_auto_centre
                                else np.zeros(DIM)
                            )
                            m1 = midpoint - shift
                            ...
                            x = [
                                x0 if pane == pane2 else None
                                for x0, pane, pane2 in zip(
                                    x, x_pane, np.roll(x_pane, 1)
                                )
                            ]
                            y = [
                                y0 if pane == pane2 else None
                                for y0, pane, pane2 in zip(
                                    y, y_pane, np.roll(y_pane, 1)
                                )
                            ]
                        else:
                            x = (
                                self.normalize(x, xmin, xmax, is_square, ymin, ymax)
                                * (SIZEX * PIXEL - 10)
                                + 5
                            )
                            y = (
                                (
                                    1
                                    - self.normalize(
                                        y, ymin, ymax, is_square, xmin, xmax
                                    )
                                )
                                * (SIZEY * PIXEL - 10)
                                + 5
                    ...
                    if is_valid:
                        if None in x or None in y:
                            for x0, y0, x1, y1 in zip(x[:-1], y[:-1], x[1:], y[1:]):
                                if not (
                                    x0 is None or y0 is None or x1 is None or y1 is None
                                ):
                                    draw.line(
                                        [x0, y0, x1, y1],
                                        fill=self.get_color(c),
    ...
    def draw_psd(self, is_welch=True):
        ...
        if self.analyzer.is_calc_psd and self.analyzer.psd_freq is not None:
            ...
            if freq is None or len(freq) == 0:
                self.draw_title(draw, 2, "collecting samples...")
                del draw
                return
            xmin, xmax = freq.min(), freq.max()
            if self.analyzer.psd1 is not None and len(self.analyzer.psd1) > 0:
                peak_freq = freq[np.argmax(self.analyzer.psd1)]
                if peak_freq > 0:
                    self.analyzer.period = 1 / peak_freq
                    self.analyzer.period_gen = self.analyzer.period * T
            for n, psd, name in zip(
                [0, 1],
                [self.analyzer.psd2, self.analyzer.psd1],
                [self.statistics_y_name, self.statistics_x_name],
            ):
                if psd is not None and psd.shape[0] > 0:
                    ...
                    self.draw_title(
                        draw,
                        3 - n,
                        "period from {stat} = {T:.2f}s".format(stat=name, T=period),
                        color=c,
                    )
                else:
                    self.draw_title(draw, 2, "collecting samples...")
    ...
    def draw_recurrence(self, e=0.1, steps=10):
        ...
        segment = np.asarray(self.analyzer.series[-1])[
            -size:, self.analyzer.RECURRENCE_RANGE
        ]
        ...
        D = (
            scipy.spatial.distance.squareform(
                np.log(scipy.spatial.distance.pdist(segment))
            )
            + np.eye(segment.shape[0]) * -100
        )
    ...
    def adjust_b(self, d):
        ...
        if B <= 0:
            ...
            self.world.params[self.show_kernel]["b"] = self.world.params[
                self.show_kernel
            ]["b"][0:5]
        elif KN == 1 and XN == 1:
    ...
    def paste_world(self):
        try:
            ...
            if "cells" in st:
                ...
                cells = np.asarray(
                    [[float(c) if c != "" else 0 for c in row] for row in reader]
                )
    ...
    def save_world(self, is_seq=False):
        ...
        try:
            ...
            with open(path + ".rle", "w", encoding="utf8") as file:
                ...
                file.write(
                    "x = "
                    + str(A.cells[0].shape[0])
                    + ", y = "
                    + str(A.cells[0].shape[1])
                    + ", rule = Lenia("
                    + A.params2st()
                    + ")\n"
                )
            ...
            data["cells"] = [
                [row if row.endswith("!") else row + "%" for row in rle.split("%")]
                for rle in data["cells"]
            ]
            ...
            with open(path + ".csv", "w", newline="\n") as file:
                ...
                writer.writerow(
                    [
                        self.analyzer.statistics_fullname(x=x)
                        for x in self.analyzer.STAT_HEADERS
                    ]
                )
    ...
    def key_press_event(self, event):
        ...
        s = (
            "s+"
            if state & 0x1 or (key.isalpha() and len(key) == 1 and key.isupper())
            else ""
        )
    ...
    def process_key(self, k):
        ...
        # sampling,symmetry [[ ] \]
            ...
            self.samp_freq = self.samp_freq + (
                4 if self.samp_freq == 1 and inc_5_or_1 == 5 else inc_5_or_1
            )
            self.info_type = "time"
            ...
            self.samp_freq = int(
                round(
                    (round(self.samp_freq / self.analyzer.period_gen) + 1)
                    * self.analyzer.period_gen
                )
            )
            self.info_type = "time"
        ...
        # params [qa ws ed rf tg]
            ...
            self.world.params[0]["T"] = max(
                1, self.world.params[0]["T"] * double_or_not + inc_or_not
            )
            self.analyzer.new_segment()
            self.info_type = "params"
        elif k in ["g", "s+g"]:
            ...
            self.world.params[self.show_kernel]["h"] = min(
                1.0, self.world.params[self.show_kernel]["h"] + 0.1
            )
            self.analyzer.new_segment()
            self.info_type = "params"
        elif k in ["c+g"]:
            self.world.params[self.show_kernel]["h"] = max(
                0.1, self.world.params[self.show_kernel]["h"] - 0.1
            )
            self.analyzer.new_segment()
            self.info_type = "params"
        elif k in ["c+r"]:
            self.world.params[self.show_kernel]["r"] = min(
                1.0, self.world.params[self.show_kernel]["r"] + 0.1
            )
            self.analyzer.new_segment()
            self.info_type = "params"
        elif k in ["c+f"]:
            self.world.params[self.show_kernel]["r"] = max(
                0.5, self.world.params[self.show_kernel]["r"] - 0.1
            )
            self.analyzer.new_segment()
            self.info_type = "params"
        elif k in ["s+c+d"]:
        ...
        # options [c+yuiop]
            ...
            self.world.params[0]["kn"] = (
                self.world.params[0]["kn"] + inc_or_dec - 1
            ) % len(self.automaton.kernel_core) + 1
            self.automaton.calc_kernel()
            self.info_type = "kn"
        elif k in ["c+u", "s+c+u"]:
            self.world.params[0]["gn"] = (
                self.world.params[0]["gn"] + inc_or_dec - 1
            ) % len(self.automaton.growth_func) + 1
            self.info_type = "gn"
        elif k in ["c+i"]:
        ...
        # slice (DIM>2) [home/end]
            ...
            self.z_slices[self.z_axis] = (
                self.z_slices[self.z_axis] + inc_10_or_1
            ) % SIZE[self.z_axis]
        ...
        # autocentre [']
            ...
            self.is_auto_centre = not self.is_auto_centre
        ...
        # animals [zxcv 1-0]
            ...
            self.found_animal_id = (
                len(self.found_animal_data) if self.found_animal_id == 0 else 0
            )
            self.load_found_animal_id(self.world, self.found_animal_id)
            self.world_updated()
            self.info_type = "animal"
        ...
        # statistics [hjkl]
            ...
            self.statistics_mode = (self.statistics_mode + inc_or_dec) % 7
            self.info_type = "statistics"
        elif k in ["k", "s+k"]:
            self.statistics_x = self.change_stat_axis(
                self.statistics_x, self.statistics_y, inc_or_dec
            )
            self.info_type = "statistics"
        elif k in ["l", "s+l"]:
            self.statistics_y = self.change_stat_axis(
                self.statistics_y, self.statistics_x, inc_or_dec
            )
            self.info_type = "statistics"
        elif k in ["c+j"]:
            self.analyzer.clear_segment()
        elif k in ["a+j"]:
            self.statistics_mode = 5  # periodogram
        elif k in ["s+c+j"]:
            ...
            self.statistics_mode = 1
            self.statistics_x_name = "m"
            self.statistics_y_name = "g"
            self.analyzer.trim_segment = 1
            self.info_type = "statistics"
        elif k in ["s+c+l"]:
            self.statistics_mode = 2
            self.statistics_x_name = "x"
            self.statistics_y_name = "y"
        ...
        self.samp_rotate = (
            (-1 if self.is_samp_clockwise else +1)
            * 360
            / self.samp_sides
            / self.samp_gen
            * self.world.params[0]["T"]
            if self.auto_rotate_mode in [3]
            else 0
        )
    ...
    def get_acc_func(self, key, acc, animal_id=None):
        ...
        ctrl = (
            "Ctrl+"
            if (is_windows or acc in ["c+Space", "c+Q", "c+H", "c+Tab"])
            else "Command+"
        )
        if acc:
            acc = (
    ...
    def create_submenu(self, parent, items):
        ...
        for i in items:
            if i is None or i == "":
                ...
                if self.is_advanced_menu or (
                    not self.is_advanced_menu and not text.startswith("*")
                ):
                    is_last_sep = False
                    text = text.lstrip("*")
                    if (text.startswith("#") and DIM == 2) or (
                        text.startswith("$") and DIM > 2
                    ):
                        key = None
                    text = text.lstrip("$#")
                    m.seq += 1
                    if first == "":
                        ...
                        self.menu_vars[name] = tk.BooleanVar(
                            value=self.get_nested_attr(name)
                        )
                        m.add_checkbutton(
                            label=text,
                            variable=self.menu_vars[name],
                            **self.get_acc_func(key, acc)
                        )
                    elif kind == "@":
                        self.menu_values[name] = (m._name, m.seq, text)
                        m.add_command(
                            label="", **self.get_acc_func(key, acc)
                        )  # background='dark green', foreground='white'
                    elif kind == "#":
                        self.menu_params[name] = (m._name, m.seq, text)
                        m.add_command(
                            label="", **self.get_acc_func(key, acc)
                        )  # state=tk.DISABLED, background='navy', foreground='white')
                    elif kind == "&":
                        m.add_command(
                            label=text, **self.get_acc_func(key, acc, animal_id=name)
                        )
        if is_last_sep:
            m.delete(m.seq)
        return m
    ...
    def get_value_text(self, name):
        ...
        if name == "animal":
            ...
            return ["Exponential", "Polynomial", "Step", "Staircase"][
                self.world.params[0].get("kn") - 1
            ]
        elif name == "gn":
            return ["Exponential", "Polynomial", "Step"][
                self.world.params[0].get("gn") - 1
            ]
            ...
            return [
                ...
                self.colormap_id
            ]  # "Pale blue/red","Pale green/purple","Pale yellow/green",
        # elif name=='show_what': return ["World","Potential","Field","Kernel","World FFT","Potential FFT","Gradient","Change"][self.show_what]
        ...
        elif name == "statistics_x":
            return self.analyzer.statistics_fullname(i=self.statistics_x)
        elif name == "statistics_y":
            return self.analyzer.statistics_fullname(i=self.statistics_y)
        elif name == "z_axis":
            ...
            return [
                ...
                "Genetic algo on max({stat})",
                "Genetic algo on avg({stat})",
                "Genetic algo on stdev({stat})",
            ][self.search_algo].format(stat=self.statistics_x_name)
    ...
    def update_menu(self):
        ...
        for name, info in self.menu_params.items():
            ...
            value = self.get_nested_attr(name)
            self.menu.children[info[0]].entryconfig(
                info[1], label="{text} [{value}]".format(text=info[2], value=value)
            )
    ...
    def create_menu(self):
        ...
        self.menu.add_cascade(
            label="Lenia",
            menu=self.create_submenu(
                self.menu,
                ["^is_run|Running|Return", "|Once|Space"]
                + (
                    [
                        "^automaton.is_gpu|Use GPU|s+c+G",
                        "|*(GPU: " + self.automaton.gpu_thr._device.name + ")|",
                    ]
                    if self.automaton.has_gpu
                ...
                + [
                    None,
                    "@show_what|Display|Tab",
                    "@show_kernel|*Kernel|QuoteLeft|`",
                    "@colormap_id|Colors|s+Period|>",
                    None,
                    "|*Show lifeform name|Comma|,",
                    "|*Show params|Period|.",
                    "|*Show info|Slash|/",
                    "|*Show auto-rotate info|s+Slash|?",
                    None,
                    "|Save data & image|c+S",
                    "|*Save next in sequence|s+c+S",
                    "^recorder.is_recording|Record video & gif|c+W",
                    "|*Record with frames saved|s+c+W",
                    None,
                    "^is_advanced_menu|Advanced menu|c+Tab",
                    None,
                    "|Quit|Escape",
                ],
            ),
        ...
        self.menu.add_cascade(
            ...
            menu=self.create_submenu(
                ...
                [
                    "|Clear|Backspace",
                    "|Random|N",
                    "|*Random (last seed)|s+N",
                    "|Random cells & params|M",
                    "|*Random cells & params (incremental)|s+M",
                    None,
                    "|Flip vertically|Equal|=",
                    "|Flip horizontally|s+Equal|+",
                    "|Mirror horizontally|c+Equal|c+=",
                    "|Mirror flip|s+c+Equal|c++",
                    "|Erase half|Minus|-",
                    None,
                    "|Copy|c+C",
                    "|*Copy as CSV|c+X",
                    "|Paste|c+V",
                    None,
                    "^is_auto_load|*Auto put mode|c+Z",
                    "^is_layer_mode|*Layer mode|s+c+X",
                ],
            ),
        )
        ...
        self.menu.add_cascade(
            ...
            menu=self.create_submenu(
                ...
                [
                    ...
                    "|*Previous found|s+B",
                    "|*Next found|B",
                    "|*First/last found|c+B",
                    "|*Delete this found|c+Backspace",
                    None,
                    "|*Start auto search (any key to stop)|c+M",
                    "@search_algo|*Search algorithm|s+c+M",
                    None,
                    "|Shortcuts 1-10|1",
                    "|Shortcuts 11-20|s+1",
                    "|Shortcuts 21-30|c+1",
        ...
        self.menu.add_cascade(
            label="List",
            menu=self.create_submenu(self.menu, self.get_animal_nested_list()),
        )
        ...
        self.menu.add_cascade(
            ...
            menu=self.create_submenu(
                self.menu,
                [
                    "^is_auto_centre|Auto-centre mode|QuoteRight|'",
                    None,
                    ...
                    "|*#Slice back|End",
                    "|*#Centre slice|c+Home",
                    "^is_show_slice|*#Show Z slice|c+End",
                    "@z_axis|*#Change Z axis|s+c+Home",
                ],
            ),
        )
        ...
        self.menu.add_cascade(
            ...
            menu=self.create_submenu(
                self.menu,
                [
                    "@polar_mode|$Polar mode|c+QuoteRight|c+'",
                    '@auto_rotate_mode|*$Auto-rotate by|s+QuoteRight|"',
                    None,
                ]
                + (
                    [
                        "|(Small adjust)||s+c+Up",
                        "|Rotate anti-clockwise|c+Up",
                        "|Rotate clockwise|c+Down",
                        None,
                    ]
                    if DIM == 2
                    else [
                        "|(Small adjust)||s+c+Up",
                        "|Rotate right|c+Right",
                        "|Rotate left|c+Left",
                        "|Rotate up|c+Up",
                        "|Rotate down|c+Down",
                        "|Rotate anti-clockwise|c+PageUp",
                        "|Rotate clockwise|c+PageDown",
                        None,
                    ]
                )
                + [
                    "|*(Small adjust)||s+]",
                    "|*Sampling period + 10|BracketRight|]",
                    "|*Sampling period - 10|BracketLeft|[",
                    "|*Clear sampling|s+BackSlash|bar",
                    "|*Run one sampling period|c+Space",
                    None,
                    "|*$Auto-rotate by sampling|BackSlash|\\",
                    "|*$Symmetry axes + 1|c+BracketRight|c+]",
                    "|*$Symmetry axes - 1|c+BracketLeft|c+[",
                    "^is_samp_clockwise|*$Clockwise|c+BackSlash|c+\\",
                ],
            ),
        )
        ...
        for i in range(5):
            ...
            items2.append(
                "|Lower peak {n}|{key}".format(n=i + 1, key="s+" + "YUIOP"[i])
            )
        # '@animal||', '#m|Field centre', '#s|Field width', '#R|Space units', '#T|Time units', '#b|Kernel peaks',
        self.menu.add_cascade(
            label="Params",
            menu=self.create_submenu(
                self.menu,
                [
                    "|(Small adjust)||s+Q",
                    "|Higher growth (m + 0.01)|Q",
                    "|Lower growth (m - 0.01)|A",
                    "|Wider growth (s + 0.001)|W",
                    "|Narrower growth (s - 0.001)|S",
                    None,
                    "|*More states (P + 10)|E",
                    "|*Fewer states (P - 10)|D",
                    "|*Reset states|c+D",
                    None,
                    "|Zoom in space (R + 10)|R",
                    "|Zoom out space (R - 10)|F",
                    "|*Reset space|s+c+R",
                    "|*Lifeform's original size|s+c+F",
                    None,
                    "|Slower time (T * 2)|T",
                    "|Faster time (T / 2)|G",
                    None,
                    "|*Larger relative kernel (r + 0.1)|c+R",
                    "|*Smaller relative kernel (r - 0.1)|c+F",
                    "|*Larger relative increment (h + 0.1)|c+T",
                    "|*Smaller relative increment (h - 0.1)|c+G",
                    None,
                    ("Peaks", items2),
                ],
            ),
        ...
        self.menu.add_cascade(
            ...
            menu=self.create_submenu(
                self.menu,
                [
                    "|*Search growth higher|c+Q",
                    "|*Search growth lower|c+A",
                    None,
                    "@kn|Kernel core|c+Y",
                    "@gn|Growth mapping|c+U",
                    None,
                    "^automaton.is_soft_clip|*Use soft clip|c+I",
                    "^automaton.is_inverted|*Invert mode|c+O",
                    "^automaton.is_arita_mode|*Target mode|c+P",
                    None,  #'^automaton.is_multi_step|*Use multi-step|c+O',
                    "@mask_rate|*Async rate|s+c+I",
                    "@add_noise|*Noise rate|s+c+O",
        ...
        self.menu.add_cascade(
            ...
            menu=self.create_submenu(
                self.menu,
                [
                    "@markers_mode|Show marks|H",
                    "^is_show_fps|*Show FPS|c+H",
                    None,
                    "@statistics_mode|Show statistics|J",
                    "@statistics_x|Statistics X axis|K",
                    "@statistics_y|Statistics Y axis|L",
                    "|*Show mass-growth|s+c+K",
                    "|*Show trajectory|s+c+L",
                    None,
                    "|*Clear segment|c+J",
                    "|*Clear all segments|s+c+J",
                    "@trim_segment|*Segment length|c+K",
                    "^is_group_params|*Group by params|c+L",
                ],
            ),
        )
    ...
    def get_info_st(self):
        ...
        if P == "0":
            P = "∞"
        return "gen={}, t={}s, dt={}s, sampl={}, P={}, world={}, pixel={}".format(
            self.automaton.gen,
            self.automaton.time,
            1 / self.world.params[0]["T"],
            self.samp_freq,
            P,
            "x".join(str(size) for size in SIZE),
            PIXEL,
        )

    def get_time_st(self):
        return "gen={}, t={}s, dt={}s, sampl={}".format(
            self.automaton.gen,
            self.automaton.time,
            1 / self.world.params[0]["T"],
            self.samp_freq,
    ...
    def get_angular_st(self):
        if self.auto_rotate_mode in [3]:
            return "auto-rotate: {} axes={} sampl={} speed={:.2f}".format(
                "clockwise" if self.is_samp_clockwise else "anti-clockwise",
                self.samp_sides,
                self.samp_gen,
                self.samp_rotate,
            )
    ...
    def update_info_bar(self):
        ...
        if self.info_type or STATUS or self.is_show_fps:
            ...
            if STATUS:
                ...
                info_st = "X axis: {xstat}, Y axis: {ystat}".format(
                    xstat=self.analyzer.statistics_fullname(i=self.statistics_x),
                    ystat=self.analyzer.statistics_fullname(i=self.statistics_y),
                )
            elif self.info_type == "slice":
                info_st = "slice: {slice}, Z axis: {d}th dim".format(
                    slice=self.z_slices, d=DIM - self.z_axis
                )
            elif self.info_type == "channel":
                info_st = "channel: {name}".format(name=self.show_which_channels_name())
            elif self.info_type == "search":
                info_st = "auto find algorithm: {algo}".format(
                    algo=self.get_value_text("search_algo")
                )
            elif self.info_type == "kernel":
                info_st = self.get_kernel_st()
            elif self.info_type in self.menu_values:
                info_st = "{text} [{value}]".format(
                    text=self.VALUE_TEXT[self.info_type],
                    value=self.get_value_text(self.info_type),
                )
    ...
    def run(self):
        ...
        while self.is_loop:
            ...
            if self.is_run:
                ...
                self.analyzer.is_calc_psd = self.statistics_mode in [5]
                self.analyzer.is_calc_symmetry = self.polar_mode not in [
                    0
                ] or self.auto_rotate_mode in [2]
                self.analyzer.centre_world()
                self.analyzer.calc_statistics(
                    self.show_what,
                    psd_x=self.statistics_x,
                    psd_y=self.statistics_y,
                    is_welch=True,
                )
                self.analyzer.add_statistics(psd_y=self.statistics_y)
                if not self.is_layer_mode and not (
                    self.search_mode == 0 and self.is_search_small
                ):
                    self.back = None
                    self.clear_transform()
            ...
            is_show_gen = (
                self.automaton.gen % 1000 == 0 and self.automaton.gen // 1000 > 3
            )
    ...
    def print_help(self):
        print(
            "Lenia in n-Dimensions    by Bert Chan 2020    Run '{program} -h' for startup arguments.".format(
                program=sys.argv[0]
            )
        )
...
if __name__ == "__main__":
    ...
    if lenia.ANIMAL_KEY_LIST != {} and lenia.ANIMAL_KEY_LIST["1"] != "":
        ...
        lenia.world.params = [
            {"R": DEF_R, "T": 10, "b": [1], "m": 0.14, "s": 0.014, "kn": 1, "gn": 1}
            for k in KERNEL
        ]
