...
class Analyzer:
    ...
    def calc_polar_FFT(self, polar_array, is_gaussian_blur=True):
        if is_gaussian_blur and PIXEL > 1:
            polar_array[:SIZER, :] = scipy.ndimage.gaussian_filter(polar_array[:SIZER, :], sigma=(2,1))
...
class Lenia:
    ...
    SAVE_ROOT = 'save'
    ANIMALS_PATH = os.path.join(os.path.dirname(__file__), '..', '_shared', 'data', 'animals.json' if DIM==2 else 'animals'+str(DIM)+'D.json')
    found_path = args.F
    ...
    def draw_world(self, A, vmin=0, vmax=1, is_shift=False, is_higher_zero=False, markers=[]):
        ...
        if self.polar_mode in [0,1]:
            ...
            if angle_shift != 0:
                self.img = self.img.rotate(-angle_shift*360, resample=PIL.Image.Resampling.NEAREST, expand=False)
            ...
            if self.is_show_rgb():
                ...
                for c in CHANNEL:
                    ...
                    for val in range(ncol):
                        ...
                        ry0, ry1 = y0+dy*val, y0+dy*(val+1)
                        draw.rectangle([(x0+c*3, min(ry0,ry1)), (x0+(c+1)*3-1, max(ry0,ry1))], fill=tuple(color))
            else:
                for c in range(ncol):
                    ry0, ry1 = y0+dy*c, y0+dy*(c+1)
                    draw.rectangle([(x0, min(ry0,ry1)), (x1, max(ry0,ry1))], fill=self.get_color(c))
            bx0, by0 = min(x0-1, x1+1), min(y0+1, y1-1)
            bx1, by1 = max(x0-1, x1+1), max(y0+1, y1-1)
            draw.rectangle([(bx0,by0),(bx1,by1)], outline=self.get_color(254))
