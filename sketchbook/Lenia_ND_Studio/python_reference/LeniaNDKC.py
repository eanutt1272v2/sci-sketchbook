...
try:
    ...
    reikna_cluda = importlib.import_module('reikna.cluda')  # pip3 install pyopencl/pycuda, reikna
    HAS_REIKNA = True
except ImportError:
    reikna_fft = None
    reikna_cluda = None
    ...
    def _recur_add(self, dim, cells1, cells2, shift, is_centreed):
        ...
        if dim < DIM-1:
            for x in range(size0):
                self._recur_add(dim+1, cells1[(start1+x)%size1], cells2[start2+x], shift, is_centreed)
        else:
            for x in range(size0):
...
class Automaton:
    ...
    def compile_gpu(self, A):
        ...
        try:
            ...
            self.gpu_fft1 = reikna_fft.FFT(A.astype(np.complex64),axes=[0]).compile(self.gpu_thr)
            self.gpu_fftn = reikna_fft.FFT(A.astype(np.complex64)).compile(self.gpu_thr)
            self.gpu_fftshift = reikna_fft.FFTShift(A.astype(np.float32)).compile(self.gpu_thr)
...
class Lenia:
    ...
    def draw_world(self, A, vmin=0, vmax=1, is_shift=False, is_higher_zero=False, markers=[]):
        ...
        if self.polar_mode in [0,1]:
            ...
            if PIXEL > 1 and self.is_auto_centre and is_shift and self.analyzer.m_centre is not None:
    ...
    def draw_arrow(self, markers=[]):
        ...
        dd = np.asarray([2]*DIM)
        if 'arrow' in markers and self.markers_mode in [0,2,4,6] and self.polar_mode in [0,1] and R > 2 and self.analyzer.m_last_centre is not None and self.analyzer.m_centre is not None:
            shift = self.analyzer.total_shift_idx if not self.is_auto_centre else np.zeros(DIM)
            m0 = self.analyzer.m_last_centre * R + midpoint + shift - self.analyzer.last_shift_idx
    ...
    def draw_symmetry(self, markers=[]):
        ...
        midpoint = np.asarray([MIDX, MIDY])
        dd = np.asarray([1, 1]) * 2
        if self.analyzer.is_calc_symmetry and 'arrow' in markers and self.markers_mode in [0,1,2,3] and R > 2 and self.analyzer.m_last_centre is not None and self.analyzer.m_centre is not None:
            is_draw_radial = self.polar_mode in [1] or (self.polar_mode in [0] and self.auto_rotate_mode in [2])
            shift = self.analyzer.total_shift_idx if not self.is_auto_centre else np.zeros(2)
            m1 = self.analyzer.m_centre * R + midpoint + shift
            ...
            if self.is_show_rgb():
                ...
                for c in CHANNEL:
                    ...
                    for val in range(ncol):
                        ...
                        draw_rect(x0+c*3, y0+dy*val, x0+(c+1)*3-1, y0+dy*(val+1), fill=tuple(color))
            else:
                for c in range(ncol):
                    draw_rect(x0, y0+dy*c, x1, y0+dy*(c+1), fill=self.get_color(c))
                ...
                for x, y, m, s, c in zip(X, Y, M, S, C):
                    ...
                    if self.is_group_params:
                        ...
                        if is_xy and self.analyzer.m_centre is not None:
                            midpoint = np.asarray(MID)
                            shift = self.analyzer.total_shift_idx + self.analyzer.m_centre * R if self.is_auto_centre else np.zeros(DIM)
                            m1 = midpoint - shift
    ...
    PARAM_TEXT = {'m':'Field centre', 's':'Field width', 'R':'Space units', 'T':'Time units', 'dr':'Space step', 'dt':'Time step', 'b':'Kernel peaks'}
    ...
    def create_menu(self):
        ...
        self.menu.add_cascade(label='Lifeform', menu=self.create_submenu(self.menu, [
            '|Place at centre|Z', '|Add at random|X',
        ...
        self.menu.add_cascade(label='Space', menu=self.create_submenu(self.menu, [
            '^is_auto_centre|Auto-centre mode|QuoteRight|\'', None,
            ...
            '|*#(Small adjust)||s+Home', '|*#Slice front|Home', '|*#Slice back|End', '|*#Centre slice|c+Home', 
