class Renderer {
  ...
  update() {
    ...
    this.lastAMu = Number(result.aMu) || this.lastAMu;
  ...
  computeGridData({
    ...
    return { grid, peak, aMu: fallbacksolver.cache?.aMu || 5.29177210903e-11 };
  }
  ...
  renderOverlay() {
    ...
    const lines = [
      ...
      `Slice ${fixedLabel}: ${sliceOffset.toFixed(2)} a_μ`,
      `View Radius: ${viewRadius.toFixed(2)} a_μ`,
      `Pan ${axis1Label}: ${viewCentre[axis1].toFixed(2)} a_μ`,
      `Pan ${axis2Label}: ${viewCentre[axis2].toFixed(2)} a_μ`,
      `Density Mean: ${this._fmtSci(stats.mean, 3)} m^-3`,
      `Density Std Dev: ${this._fmtSci(stats.stdDev, 3)} m^-3`,
      `Density Peak: ${this._fmtSci(stats.peakDensity, 3)} m^-3`,
      `Entropy: ${stats.entropy.toFixed(4)}`,
      `Concentration: ${stats.concentration.toFixed(4)}`,
      `Radial Peak: ${stats.radialPeak.toFixed(3)} a_μ`,
      `Radial Spread: ${stats.radialSpread.toFixed(3)} a_μ`,
